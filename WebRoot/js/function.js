...
/*function saveScore(score){
	console.log("begin+lhgdialog");
	lhgdialog({ 
	    content: 'url:<%=basePath%>report/report!reportSave.action?score='+score
	});	
...
function saveScore(score){
	...
	$.ajax({
		url:"report/report!reportSave.action",
		dataType:"text",	
		data:{score:score},
		
		success:(function(data){
			alert("用户总分保存成功！分数为："+score+"分");
		})
	});
...
function int(){
	...
	$("#check_answer").click(function(){
		...
		$("h4 span").show();
		var score = (tall-err)/ tall * 100;
		$("#status").html("错误：" + err + "个！错误率：" +  (err / tall * 100).toFixed(2) + "%！"+ "总分："+score+"分！");
...
function get_topic(_arr_topic,_topic_num){
	...
	for(y = 0; y < _topic_num; y++){
		...
		/*console.log("_topic+"+i+":"+" "+_topic);*/
		
		_arr_answer = _topic_temp[0][1];//答案数组
		console.log("_arr_answer+"+i+":"+" "+_arr_answer);
		console.log("_topic_temp[0][1]+"+i+":"+" "+_topic_temp[0][2]);
		_arr_answer_length = _arr_answer.length; //答案长度
		
		//获得答案字符串 begin  _answer_str  _answer_this_tip
		_answer_str = "";
		if(_arr_answer_length >2){		
			...
			try{
				console.log("_arr_answer[0][1]:"+_arr_answer[0][1]);
				console.log("_arr_answer[1][1]:"+_arr_answer[1][1]);
